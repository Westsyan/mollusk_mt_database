...
var obj = (function() {
	...
	var picture_list = $('div.picture_list');
	// area_img 显示上下箭头盒子
	...
	// 上下箭头的图片路径
	var imgsrc = ['fa fa-minus', 'fa fa-plus'];
	// 创建空数组，记录每个栏目距离顶部的距离
	var arrtop = [];
	// 计数器,当前滚动到第几个栏目;
	var cont = 0;
	...
	return {
		...
		istoge: function() {
			...
			if (m.siblings(picture_list.selector).is(':hidden')) {
				m.children(area_img.selector).children().attr('class', imgsrc[0]);
			} else {
				m.children(area_img.selector).children().attr('class', imgsrc[1]);
			}
		...
		onetoge: function() {
			...
			if (boxlen.eq(thisidx).find(picture_list.selector).is(':hidden')){
				// 点击固定滚动的栏目切换箭头图标
				m.find(area_img.selector).children().class('src', imgsrc[0]);
				// 点击固定滚动的栏目，对应的box盒子切换箭头图标
				combox.children().eq(thisidx).find(area_img.selector).children().attr('class', imgsrc[0]);
			}else {
				// 点击固定滚动的栏目切换箭头图标
				m.find(area_img.selector).children().class('src', imgsrc[1]);
				// 点击固定滚动的栏目，对应的box盒子切换箭头图标
				combox.children().eq(thisidx).find(area_img.selector).children().attr('class', imgsrc[1]);
			}
		...
		istitle: function() {
			...
			// 判断当前状态是否展开
			if (boxlen.eq(thisidx).find(picture_list.selector).is(':hidden')) {
				fixedbox.find(area_img.selector).children().attr('class', imgsrc[1]);
			} else {
				fixedbox.find(area_img.selector).children().attr('class', imgsrc[0]);
			}
		}
	}
}());
