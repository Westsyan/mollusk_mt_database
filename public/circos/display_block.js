...
// set the svg and other parameters for drawing synteny genes
	var svg = d3.select("#svg"),
		margin = { left:150, right:90, top:100, bottom:100},
		wsvg  = +svg.attr("width"),
  ...
  var drawFunction = function(){
	...
	textA.enter().append('text')
		...
		.html(function(d){
			if (d == 0) {
				return "<a href=/RiceRC/riceInfo/moreInfoByGeneId?geneId=" + data_A[d][0] + " target=_blank>" + data_A[d][0] + "</a>";
			}
	...
	textB.enter().append('text')
		...
		.html(function(d){
			if (d == 0) {
				return "<a href=/RiceRC/Msu/MSUMoreInfoByGeneId?geneId=" + data_B[d][0] + " target=_blank>" + data_B[d][0] + "</a>";
			}
